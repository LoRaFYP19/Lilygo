/*
   ...
   With additional Capabilities to reconfigure and healthping
*/
...
// #define repeatSF 8 // using single SF mode
...
unsigned long previousMillis_pkt_cache_reset = 0;
unsigned long timeout_pkt_cache_reset=60000;
...
void startReTransmission() {
    ...
    previousMillis_pkt_cache_reset= millis();
}
...
void loop(){
    ...
    if ((scheduled == true) && (currentMillis - previousMillis >= interval)) { // if scheduled and time is reached, and not transmitting
        setup_old_lora();
    ...
    else if ((scheduled_health == true) && (currentMillis - previousMillis_health >= interval_health)) { // if scheduled and time is reached and no current transmissions
    ...
    if (actionFlag){
        ...
        if (isTransmitting) {
            ...
            if (u8g2) {
                char buf[256];
                u8g2->clearBuffer();
                u8g2->drawStr(0, 12, "ReTransmitting: OK!");
 ...
 if (currentMillis- previousMillis_pkt_cache_reset>= timeout_pkt_cache_reset) {
